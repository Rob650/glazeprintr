...
def _analyze_platform_launches(search_term: str) -> dict:
    ...
    recent = [
        p for p in pairs
        if p.get("chainId") == "solana" and (p.get("pairCreatedAt") or 0) > cutoff_ms
    ]
    ...
    for pair in recent:
        ...
        age_hours = (time.time() * 1000 - created_ms) / 3_600_000 if created_ms else 24.0

        is_rug = (change_24h <= _RUG_PRICE_DROP_PCT) or (liq < _RUG_MIN_LIQUIDITY and age_hours >= _RUG_MIN_AGE_HOURS)
        if is_rug:
            rugs += 1
        else:
            survival_hours.append(min(age_hours, 24.0))
...
def refresh_competitor_stats() -> None:
    ...
    for platform, search_term in _PLATFORMS.items():
        try:
            stats = _analyze_platform_launches(search_term)
